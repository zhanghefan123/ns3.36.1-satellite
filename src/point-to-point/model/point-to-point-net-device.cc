...
PointToPointNetDevice::GetTypeId (void)
{
  static TypeId tid =
    ...
    .AddTraceSource ("PhyRxBegin", 
                     ...
                     "ns3::Packet::TracedCallback")
#endif
          // MakeTraceSourceAccessor 用来创建一个TraceSourceAccessor,他将控制对于底层跟踪源的访问
...
PointToPointNetDevice::TransmitStart (Ptr<Packet> p)
{
  ...
  // NetDevice 的传输状态是READY状态，一定要断言是READY状态
  NS_ASSERT_MSG (m_txMachineState == READY, "Must be READY to transmit");
  m_txMachineState = BUSY;
  // 将p放到m_currentPkt之中
  m_currentPkt = p;
  // 当数据包在介质上开始传输过程时触发的trace,ns3::Packet::TracedCallBack
  // TracedCallBack是一个函数指针，这个函数指针指向的函数的签名有一个参数：这个参数接收一个数据包
  m_phyTxBeginTrace (m_currentPkt);
  // 进行传输延迟的计算
  Time txTime = m_bps.CalculateBytesTxTime (p->GetSize ());
  // m_tInterframeGap 是 netdevice 用来限制数据包的帧间间隙的 Time
